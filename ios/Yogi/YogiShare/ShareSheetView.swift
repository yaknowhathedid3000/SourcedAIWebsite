...
struct ShareSheetView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            card
                .background(YogiColor.ground, in: RoundedRectangle(cornerRadius: 28, style: .continuous))
    ...
    private var card: some View {
        VStack(spacing: 0) {
            Capsule().fill(YogiColor.hairline).frame(width: 42, height: 5).padding(.top, 10)

            HStack {
                Text("Save to Yogi").font(.yogiDisplay(28)).foregroundStyle(YogiColor.ink)
                Spacer()
                Button(action: onCancel) {
                    Image(systemName: "xmark")
                        ...
                        .background(YogiColor.optionFill, in: Circle())
                }
                .buttonStyle(.plain)
            }
            ...
            Text("Yogi will read it properly when you open the app.")
                .font(.yogiSans(13))
                ...
                ForEach(SaveCategory.browsable) { c in
                    ...
                    Button {
                        ...
                        HStack(spacing: 7) {
                            ...
                            Text(c.title).font(.yogiSans(16, weight: .medium))
                        }
                        .foregroundStyle(selected ? Color.white : YogiColor.ink)
                        .padding(.horizontal, 15)
                        .frame(height: 42)
                        .background(selected ? YogiColor.ink : YogiColor.optionFill, in: Capsule())
                    }
                    .buttonStyle(.plain)
                }
            }
