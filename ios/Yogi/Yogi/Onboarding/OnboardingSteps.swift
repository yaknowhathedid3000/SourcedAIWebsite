    ...
    var body: some View {
        ZStack {
            if showResult {
                ...
                VStack(spacing: 18) {
                    Text("How to save to Yogi").yogiText(.onboardingHeadline)
                    HStack(spacing: 14) {
                        ForEach(script.platformEmoji, id: \.self) { e in
                            Text(e).font(.system(size: 22)).frame(width: 52, height: 52).background(YogiColor.optionFill, in: Circle())
                        }
                    }
...
struct DemoFrame: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            case "post":
                VStack(spacing: 0) {
                    ...
                    HStack { Text("kaltpark  japan, you're silly delicious").font(.yogiSans(11)); Spacer() }.padding(12)
                    Text("Add comment...").font(.yogiSans(11)).foregroundStyle(YogiColor.muted).frame(maxWidth: .infinity, alignment: .leading).padding(.horizontal, 12).padding(.bottom, 16)
                }
                VStack(alignment: .trailing, spacing: 10) {
                    Spacer()
                    Text("♥ 14.8K").font(.yogiSans(11, weight: .bold))
                    Text("💬 58").font(.yogiSans(11, weight: .bold))
                    Text("➤ 1,404").font(.yogiSans(11, weight: .bold)).padding(6).background(Circle().stroke(YogiColor.systemBlue, lineWidth: 2))
            ...
            case "page":
                VStack(alignment: .leading, spacing: 8) {
                    Text("CHEW out loud").font(.yogiSans(12, weight: .black)).padding(.top, 46)
                    Text("Easy Jamaican Jerk Chicken Recipe").font(.yogiSans(18, weight: .bold))
                    Text("By Amy Dong · Updated Apr. 22, 2024 · 378 ratings").font(.yogiSans(10)).foregroundStyle(YogiColor.muted)
                    RoundedRectangle(cornerRadius: 10).fill(Color(hex: 0x7B3F00)).frame(height: 130).overlay(Text("🍗").font(.system(size: 54)))
                    Text("JUMP TO RECIPE").font(.yogiSans(11, weight: .bold)).padding(8).background(YogiColor.optionFill, in: Capsule())
                    Spacer()
                    HStack { Spacer(); Text("chewoutloud.com").font(.yogiSans(10)); Spacer(); Text("···").font(.yogiSans(18, weight: .bold)) }.padding(.bottom, 14)
                }
            ...
            case "appShare", "browserMenu":
                VStack(spacing: 10) {
                    Spacer()
                    VStack(spacing: 8) {
                        Capsule().fill(YogiColor.hairline).frame(width: 36, height: 4)
                        if name == "appShare" {
                            HStack(spacing: 14) {
                                ForEach(["Fleur", "Madeline", "hero", "ARCHIF"], id: \.self) { n in
                                    VStack { Circle().fill(YogiColor.optionFill).frame(width: 40, height: 40); Text(n).font(.yogiSans(9)) }
                                }
                            }
                            HStack(spacing: 14) {
                                ForEach(["Add to story", "WhatsApp", "Status", "Share to...", "Copy Link"], id: \.self) { n in
                                    VStack { Circle().fill(n == "Share to..." ? YogiColor.systemBlue.opacity(0.2) : YogiColor.optionFill).frame(width: 40, height: 40); Text(n).font(.yogiSans(8)).lineLimit(1) }
                                }
                            }
                        } else {
                            ForEach(["Share", "Add to Bookmarks", "Add to Reading List", "New Tab", "New Private Tab"], id: \.self) { n in
                                HStack { Text(n).font(.yogiSans(13)); Spacer() }.padding(.horizontal, 14).frame(height: 30)
                                    .background(n == "Share" ? YogiColor.systemBlue.opacity(0.12) : Color.clear)
                            }
                        }
                    }
                    .padding(.vertical, 12)
                    .frame(maxWidth: .infinity)
                    .background(YogiColor.card, in: RoundedRectangle(cornerRadius: 20))
                    .padding(8)
                }
                .foregroundStyle(YogiColor.ink)
            ...
            case "iosShare":
                VStack(spacing: 10) {
                    ...
                    VStack(spacing: 12) {
                        HStack(spacing: 18) {
                            ForEach(["AirDrop", "Messages", "Mail", "Yogi"], id: \.self) { n in
                                VStack(spacing: 4) {
                                    ZStack {
                                        RoundedRectangle(cornerRadius: 12).fill(n == "Yogi" ? YogiColor.brandOrange : YogiColor.optionFill).frame(width: 48, height: 48)
                                        if n == "Yogi" { MascotView(color: .white).frame(width: 26) }
                                    }
                                    Text(n).font(.yogiSans(9))
                                }
                            }
                        }
                        ForEach(["📁  Save to Files", "🖨️  Print"], id: \.self) { n in
                            HStack { Text(n).font(.yogiSans(13)); Spacer() }.padding(.horizontal, 14).frame(height: 32).background(YogiColor.optionFill, in: RoundedRectangle(cornerRadius: 10)).padding(.horizontal, 8)
                        }
                    }
                    .padding(.vertical, 14)
                    .frame(maxWidth: .infinity)
        ...
        .background(name == "post" ? Color(hex: 0x8D99AE) : YogiColor.surface)
    }
}
    ...
    var body: some View {
        ZStack {
            OnboardingPage(buttonTitle: "Continue", centered: true, onContinue: onContinue) {
                VStack(spacing: 6) {
                    Text(script == .instagram ? "12 places found in Instagram video" : "1 recipe found in web link").yogiText(.onboardingHeadline).multilineTextAlignment(.center)
                    Text("Just like magic!").font(.yogiSans(18)).foregroundStyle(YogiColor.muted)
                    YogiCard {
                        if script == .instagram {
                            VStack(alignment: .leading, spacing: 14) {
                                Text("Japan Food Guide").font(.yogiSans(22, weight: .bold))
                                MiniMap(pins: ["☕️", "🍣", "🍜", "🏪"])
                                ForEach(Array(zip(["Suba", "Yakumo", "Iseya Sohonten", "Warito"], ["🇯🇵 Soba noodle shop · 3.7 · $$", "🍜 Ramen restaurant · 4.2 · $$", "🍶 Izakaya restaurant · 3.7 · $$", "🍢 Skewers · 4.0 · $"])), id: \.0) { name, meta in
                                    HStack(spacing: 14) {
                                        RoundedRectangle(cornerRadius: 10).fill(YogiColor.optionFill).frame(width: 64, height: 84).overlay(Text("🏮").font(.system(size: 28)))
                                        VStack(alignment: .leading, spacing: 4) {
                                            Text(name).font(.yogiSans(18, weight: .bold))
                                            Text(meta).font(.yogiSans(15)).foregroundStyle(YogiColor.inkSecondary)
                                        }
                                    }
                                }
                            }
                            ...
                            VStack(alignment: .leading, spacing: 14) {
                                Text("Easy Jamaican Jerk Chicken").font(.yogiSans(22, weight: .bold))
    ...
    var body: some View {
        ZStack {
            ...
            ForEach(Array(pins.enumerated()), id: \.offset) { i, e in
                Text(e).font(.system(size: 26)).padding(6).background(YogiColor.card, in: Circle()).shadow(color: .black.opacity(0.15), radius: 6, y: 3)
...
struct NotificationsStep: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            VStack(alignment: .leading, spacing: 8) {
                Text("Don't let a good save go to waste").yogiText(.onboardingHeadline)
                Text("Let Yogi will remind you about events and limited time offers.").font(.yogiSans(17)).foregroundStyle(YogiColor.inkSecondary)
            }
            .frame(maxWidth: .infinity, alignment: .leading)
            .padding(.horizontal, 20)
            Spacer()
            ZStack {
                RoundedRectangle(cornerRadius: 24, style: .continuous).fill(YogiColor.card).frame(width: 180, height: 180)
        ...
        .background(YogiColor.surface.ignoresSafeArea())
    }
}
...
struct BuildingPlanStep: View {
    ...
    private let phases = ["Building your watchlist...", "Pinning your places...", "Extracting your recipes...", "Making Yogi yours"]
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            .frame(height: 300)
            .padding(.top, 30)
            Text("Making Yogi yours").yogiText(.onboardingHeadline)
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    Capsule().fill(YogiColor.optionFill)
                    Capsule().fill(LinearGradient(colors: [Color(hex: 0x8EC5FF), YogiColor.systemBlue], startPoint: .leading, endPoint: .trailing))
            ...
            .padding(.top, 16)
            Text(phases[min(phase, phases.count - 1)]).font(.yogiSans(17)).foregroundStyle(YogiColor.inkSecondary).padding(.top, 12)
            VStack(spacing: 0) {
                HStack {
                    Text("Setting up your Yogi for").font(.yogiSans(20, weight: .bold))
                    Spacer()
                    Text("\(Int(progress * 100))%").font(.yogiSans(20, weight: .bold)).monospacedDigit()
                }
                .padding(.bottom, 10)
                Divider()
                ForEach(Array(lines.enumerated()), id: \.offset) { i, line in
                    HStack {
                        Text(line).font(.yogiSans(18))
                        Spacer()
                        if i < completed {
                            Image(systemName: "checkmark.circle.fill").font(.system(size: 26)).foregroundStyle(YogiColor.ink).transition(.scale)
                        } else {
                            ProgressView().tint(YogiColor.muted)
                        }
                    }
                    .frame(height: 50)
                }
            ...
            VStack(spacing: 28) {
                GeometryReader { geo in
                    ...
                    ZStack {
                        ...
                        .stroke(YogiColor.hairline, lineWidth: 1)
                        // Area
                        ...
                        .fill(LinearGradient(colors: [YogiColor.systemBlue.opacity(0.18), .clear], startPoint: .top, endPoint: .bottom))
                        // Curve
                        curve(start: start, end: end)
                            .trim(from: 0, to: drawn)
                            .stroke(LinearGradient(colors: [YogiColor.danger, YogiColor.brandOrange, YogiColor.systemBlue], startPoint: .leading, endPoint: .trailing),
                        ...
                        .stroke(YogiColor.muted.opacity(0.5), style: StrokeStyle(lineWidth: 1.5, dash: [4, 4]))
                        Circle().fill(YogiColor.danger).frame(width: 16, height: 16).position(start)
                        Circle().fill(YogiColor.systemBlue).frame(width: 16, height: 16).position(end)
                        Text("Endless\nDoomscrolling").font(.yogiSans(15, weight: .semibold)).foregroundStyle(.white).multilineTextAlignment(.leading)
                            .padding(.horizontal, 14).padding(.vertical, 10).background(YogiColor.danger, in: RoundedRectangle(cornerRadius: 12))
                            .position(x: start.x, y: start.y - 52)
                        Text("Booked\n& Busy").font(.yogiSans(15, weight: .semibold)).foregroundStyle(.white).multilineTextAlignment(.center)
                            .padding(.horizontal, 14).padding(.vertical, 10).background(YogiColor.systemBlue, in: RoundedRectangle(cornerRadius: 12))
                            .position(x: end.x, y: end.y - 52)
                        Group {
                            Text("Now").font(.yogiSans(15, weight: .semibold)).foregroundStyle(YogiColor.danger).position(x: start.x, y: h * 0.94)
                            Text("Your goal").font(.yogiSans(15, weight: .semibold)).foregroundStyle(YogiColor.systemBlue).position(x: end.x, y: h * 0.94)
                        }
                    }
                }
                .frame(height: 380)
...
struct GridMockup: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(title).font(.yogiDisplay(18)).padding(.top, 44)
            Text("All the \(title.lowercased().replacingOccurrences(of: "saved ", with: "")) found in your library").font(.yogiSans(9)).foregroundStyle(YogiColor.muted)
            HStack { Text("List").font(.yogiSans(10)); Text("Grid").font(.yogiSans(10, weight: .bold)); Spacer(); Text("Select").font(.yogiSans(10)) }
...
struct NoPaymentDueNow: View {
    var body: some View {
        HStack(spacing: 10) {
            ...
            Text("No Payment Due Now").font(.yogiSans(18))
        }
        .foregroundStyle(YogiColor.ink)
    }
}
        ...
        OnboardingPage(buttonTitle: "Continue", centered: true, onContinue: onContinue) {
            VStack(spacing: 28) {
                ...
                (Text("You'll get a reminder ") + Text("1 day").fontWeight(.bold) + Text(" before your trial ends."))
                    .font(.yogiSans(26)).foregroundStyle(YogiColor.ink).multilineTextAlignment(.center)
                NoPaymentDueNow()
            }
        }
    }
...
struct WelcomeClubStep: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            ZStack {
                VStack(spacing: 18) {
                    Capsule().fill(YogiColor.hairline).frame(width: 40, height: 5)
                    MascotView(variant: .king).frame(width: 190, height: 190).padding(.top, 10)
                    Text("Welcome to the\nDigital Hoarder's Club!").font(.yogiSans(28, weight: .bold)).multilineTextAlignment(.center).foregroundStyle(YogiColor.ink)
                    Text("You've unlocked unlimited bulk imports and premium features. Time to save everything!")
                        .font(.yogiSans(17)).foregroundStyle(YogiColor.inkSecondary).multilineTextAlignment(.center).padding(.horizontal, 16)
            ...
            .frame(maxWidth: .infinity)
            .background(YogiColor.card, in: UnevenRoundedRectangle(topLeadingRadius: 36, bottomLeadingRadius: 0, bottomTrailingRadius: 0, topTrailingRadius: 36, style: .continuous))
