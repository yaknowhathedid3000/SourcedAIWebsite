...
struct CopyableHeading: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            Text(title).font(.yogiSans(24, weight: .bold)).foregroundStyle(YogiColor.ink)
            Button {
                UIPasteboard.general.string = payload
                copied = true
                Task { try? await Task.sleep(for: .seconds(1.5)); copied = false }
            } label: {
                Image(systemName: copied ? "checkmark" : "doc.on.doc").font(.system(size: 16)).foregroundStyle(YogiColor.inkSecondary)
            }
            .buttonStyle(.plain)
...
struct IconField: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            Image(systemName: systemImage).font(.system(size: 20)).foregroundStyle(disabled ? YogiColor.muted : YogiColor.inkSecondary).frame(width: 26)
            TextField(placeholder, text: $text)
                .font(.yogiSans(18))
                .foregroundStyle(disabled ? YogiColor.muted : YogiColor.ink)
        ...
        .background(disabled ? YogiColor.surface : YogiColor.optionFill, in: RoundedRectangle(cornerRadius: 18, style: .continuous))
    }
}
    ...
    var body: some View {
        VStack(alignment: .trailing, spacing: 6) {
            ZStack(alignment: .topLeading) {
                if text.isEmpty {
                    Text(placeholder).font(.yogiSans(18)).foregroundStyle(YogiColor.muted).padding(.horizontal, 18).padding(.top, 16)
                }
            ...
            Text("\(text.count)/\(limit)").font(.yogiSans(14)).foregroundStyle(YogiColor.muted).monospacedDigit()
        }
    }
}
...
struct SheetHeader: View {
    ...
    var body: some View {
        VStack(spacing: 14) {
            ...
            if let title {
                ZStack {
                    Text(title).font(.yogiDisplay(26)).foregroundStyle(YogiColor.ink)
                    if let onBack {
                        HStack {
                            Button(action: onBack) { Image(systemName: "chevron.left").font(.system(size: 22, weight: .semibold)).foregroundStyle(YogiColor.ink).frame(width: 40, height: 40) }
                                .buttonStyle(.plain)
...
struct SearchPill: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            if let leadingEmoji { Text(leadingEmoji) } else { Image(systemName: "magnifyingglass").font(.system(size: 20)).foregroundStyle(YogiColor.inkSecondary) }
            TextField(placeholder, text: $text).font(.yogiSans(18)).foregroundStyle(YogiColor.ink)
            if !text.isEmpty {
                Button { text = ""; onClear?() } label: { Image(systemName: "xmark.circle.fill").font(.system(size: 22)).foregroundStyle(YogiColor.inkSecondary) }
                    .buttonStyle(.plain)
            }
        }
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 8) {
                ...
                Text(title).font(.yogiSans(17, weight: .medium))
            }
            .foregroundStyle(YogiColor.ink)
    ...
    var body: some View {
        Button(action: action) {
            Text(isFollowing ? "Following" : "Follow")
                ...
                .foregroundStyle(isFollowing ? YogiColor.ink : .white)
                .padding(.horizontal, 24).frame(height: 52)
                .frame(maxWidth: expands ? .infinity : nil)
                .background {
                    if isFollowing {
                        RoundedRectangle(cornerRadius: 16, style: .continuous).stroke(YogiColor.hairline, lineWidth: 1.5)
                    } else {
                        ZStack {
                            RoundedRectangle(cornerRadius: 16, style: .continuous).fill(Color.black).offset(y: 4)
                            RoundedRectangle(cornerRadius: 16, style: .continuous).fill(YogiColor.ink)
                        }
                    }
                }
        }
