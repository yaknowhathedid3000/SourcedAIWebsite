...
enum YogiAppGroup {
    static let identifier = "group.com.sourcedai.yogi"
...
/// One thing handed to Yogi from another app's share sheet.
...
struct InboxItem: Codable, Identifiable, Hashable {
    ...
    /// Filenames inside `YogiAppGroup.attachments`.
...
enum ShareInbox {
    private static let key = "yogi.shareInbox"

    static func pending() -> [InboxItem] {
        guard let defaults = YogiAppGroup.defaults,
    ...
    static func clear() {
        write([])
        if let dir = YogiAppGroup.attachments {
    ...
    static func storeAttachment(_ data: Data, ext: String = "jpg") -> String? {
        guard let dir = YogiAppGroup.attachments else { return nil }
    ...
    static func discardAttachments(_ names: [String]) {
        guard let dir = YogiAppGroup.attachments else { return }
        for name in names {
