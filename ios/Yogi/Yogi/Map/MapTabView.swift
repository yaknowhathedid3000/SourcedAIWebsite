        ...
        ZStack(alignment: .bottom) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .bottom) {
                    Menu {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Text(filter.title).font(.yogiSans(17, weight: .semibold))
                            Image(systemName: "chevron.down").font(.system(size: 13, weight: .semibold))
                        }
                        .foregroundStyle(filter == .all ? YogiColor.ink : .white)
                        .padding(.horizontal, 18).frame(height: 50)
                        .background(filter == .all ? YogiColor.card : YogiColor.ink, in: Capsule())
                        .shadow(color: .black.opacity(0.15), radius: 10, y: 4)
                    }
                    Spacer()
                    VStack(spacing: 14) {
                        FloatingButton(action: { showList = true }) { Image(systemName: "building.2").font(.system(size: 20)).foregroundStyle(YogiColor.ink) }
                            .accessibilityLabel("Places list")
                        FloatingButton(action: recenter) { Image(systemName: "location").font(.system(size: 20)).foregroundStyle(YogiColor.ink) }
                ...
                HStack(spacing: 12) {
                    ...
                    if !query.isEmpty { Button { query = "" } label: { Image(systemName: "xmark").font(.system(size: 18, weight: .semibold)).foregroundStyle(YogiColor.ink) } }
                }
    ...
    var body: some View {
        VStack(spacing: 4) {
            Text(save.place?.countryFlag ?? save.place?.emoji ?? "📍").font(.system(size: 26))
                .padding(6).background(YogiColor.card, in: RoundedRectangle(cornerRadius: 10, style: .continuous))
                .overlay(RoundedRectangle(cornerRadius: 10, style: .continuous).stroke(save.status == .wantTo ? YogiColor.systemBlue : YogiColor.hairline, lineWidth: 2))
                .shadow(color: .black.opacity(0.2), radius: 6, y: 3)
            VStack(spacing: 0) {
                if save.status == .wantTo { Text("WANT TO GO").font(.system(size: 9, weight: .bold)).italic().tracking(0.5) }
                Text(save.title).font(.yogiSans(13, weight: .bold))
            }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 18) {
                ...
                Text(orphanSafe: "Enable Location for Better Experience").font(.yogiSans(22, weight: .bold)).foregroundStyle(YogiColor.ink).multilineTextAlignment(.center)
                VStack(alignment: .leading, spacing: 10) {
                    Text("We use your location to:").font(.yogiSans(16, weight: .semibold)).foregroundStyle(YogiColor.ink)
                    ForEach(["Show places near you", "Calculate distances to saved locations", "Provide personalized recommendations"], id: \.self) { t in
                        HStack(spacing: 10) { Circle().fill(YogiColor.systemBlue).frame(width: 6, height: 6); Text(t).font(.yogiSans(15)).foregroundStyle(YogiColor.inkSecondary) }
                    }
                }
                .frame(maxWidth: .infinity, alignment: .leading)
                HStack(spacing: 10) {
                    Image(systemName: "shield.lefthalf.filled").font(.system(size: 18)).foregroundStyle(YogiColor.systemBlue)
                    Text("Your location data stays private and is never shared").font(.yogiSans(14)).foregroundStyle(YogiColor.inkSecondary)
                }
                        ...
                        Button { onSelect(s) } label: {
                            HStack(spacing: 14) {
                                ...
                                VStack(alignment: .leading, spacing: 6) {
                                    Text(s.title).font(.yogiSans(20, weight: .bold)).foregroundStyle(YogiColor.ink).lineLimit(1)
                                    Text(s.metaLine).font(.yogiSans(16)).foregroundStyle(YogiColor.inkSecondary).lineLimit(1)
                                    HStack(spacing: 6) {
                                        AvatarStack(users: s.savedBy.isEmpty ? [SampleData.isaac, SampleData.jake] : s.savedBy, size: 22)
                                        Text("\(s.saveCount) saves").font(.yogiSans(15)).foregroundStyle(YogiColor.inkSecondary)
                                    }
                                }
                                Spacer()
                                Image(systemName: "megaphone").font(.system(size: 18)).foregroundStyle(YogiColor.ink).frame(width: 52, height: 52).background(Circle().stroke(YogiColor.hairline, lineWidth: 1.5))
                            }
                            .padding(.vertical, 12)
                        }
                        .buttonStyle(.plain)
        ...
        if let s = app.save(saveID), let p = s.place {
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    Text(s.title).yogiText(.screenTitle)
                    if let flag = p.countryFlag { Text(flag).font(.system(size: 30)) }
                    HStack(spacing: 10) {
                        ActionChip(systemImage: "safari", title: "Directions") { openDirections(p) }
                        if let site = p.website { Link(destination: site) { HStack(spacing: 8) { Image(systemName: "link"); Text("Website").font(.yogiSans(17, weight: .medium)) }.foregroundStyle(YogiColor.ink).padding(.horizontal, 18).frame(height: 52).background(YogiColor.optionFill, in: RoundedRectangle(cornerRadius: 14, style: .continuous)) } }
                        ActionChip(systemImage: "ellipsis", title: "More") { showActions = true }
                    }
                    Text(p.hoursLabel ?? "No opening times found").font(.yogiSans(18)).foregroundStyle(YogiColor.muted)
                    ...
                    HStack(spacing: 12) {
                        ...
                        Text("\(s.saveCount) saves").font(.yogiSans(16)).foregroundStyle(YogiColor.inkSecondary)
                        ForEach(s.reactions) { r in
                            HStack(spacing: 6) { Text(r.emoji); Text("\(r.count)") }.font(.yogiSans(15)).foregroundStyle(YogiColor.ink).padding(.horizontal, 12).frame(height: 36).background(YogiColor.optionFill, in: Capsule())
                        }
                        Spacer()
                        Button("Open") { showDetail = true }.font(.yogiSans(16, weight: .semibold)).foregroundStyle(YogiColor.ink)
                    }
                }
