...
struct ReviewFormView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    HStack(spacing: 12) {
                        Image(systemName: "megaphone.fill").font(.system(size: 28)).foregroundStyle(YogiColor.ink)
                        Text("How was it?").yogiText(.sheetTitle)
                    }
                    if stage == 0 {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            Text("Title").font(.yogiSans(15)).foregroundStyle(YogiColor.muted)
                            TextField("What did you love about it?", text: $title, axis: .vertical).font(.yogiSans(18)).lineLimit(1...3)
                        ...
                        HStack(spacing: 10) {
                            Button { pickerDate = completedOn ?? Date(); showDatePicker = true } label: {
                                ...
                                .font(.yogiSans(16, weight: .medium)).foregroundStyle(completedOn == nil ? YogiColor.inkSecondary : .white)
                                .padding(.horizontal, 16).frame(height: 44)
                                .background(completedOn == nil ? YogiColor.optionFill : YogiColor.ink, in: Capsule())
                            }
                            .buttonStyle(PressableButtonStyle())
                            Button { completedOn = nil } label: {
                                Text("Don't remember").font(.yogiSans(16, weight: .medium)).foregroundStyle(completedOn == nil ? .white : YogiColor.inkSecondary)
                                    .padding(.horizontal, 16).frame(height: 44)
                                    .background(completedOn == nil ? YogiColor.ink : YogiColor.optionFill, in: Capsule())
                            }
                            .buttonStyle(PressableButtonStyle())
                        }
                        HStack(spacing: 12) {
                            ForEach(0..<photos, id: \.self) { i in
                                RoundedRectangle(cornerRadius: 12).fill(YogiColor.optionFill).frame(width: 96, height: 128)
                            ...
                            Button { showPhotoSource = true } label: {
                                RoundedRectangle(cornerRadius: 12).stroke(YogiColor.muted, style: StrokeStyle(lineWidth: 2, dash: [6, 5]))
                                    ...
                                    .overlay(
                                        VStack(spacing: 6) {
                                            ...
                                            if photos == 0 { Text("Add photos").font(.yogiSans(16)) }
                                        }
                        ...
                        Button { showTag = true } label: {
                            HStack {
                                ...
                                Text(tagged.isEmpty ? "Tag people you did this with" : "With " + tagged.map(\.name).joined(separator: ", ")).font(.yogiSans(17))
                                Spacer()
                        ...
                        Toggle(isOn: $friendsOnly) {
                            HStack(spacing: 10) {
                                ...
                                Text("Friends only").font(.yogiSans(17))
                            }
                            .foregroundStyle(YogiColor.ink)
                        }
        ...
        .sheet(isPresented: $showDatePicker) {
            VStack(spacing: 16) {
                Text("Select date").font(.yogiSans(20, weight: .bold))
    ...
    func makeBody(configuration: Configuration) -> some View {
        Button { configuration.isOn.toggle() } label: {
            HStack {
                configuration.label
                Spacer()
                Image(systemName: configuration.isOn ? "checkmark.square.fill" : "square").font(.system(size: 26)).foregroundStyle(YogiColor.ink)
            }
        }
        .buttonStyle(.plain)
    }
