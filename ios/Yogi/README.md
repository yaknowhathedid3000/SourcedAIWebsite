...
Double-click `Yogi.xcodeproj`. Requires Xcode 15.4 or newer and iOS 17; nothing else to
install. Wait for the Supabase package to resolve on first open before building.

Two targets: `Yogi` (the app) and `YogiShare` (the "Add to Yogi" share extension). Set your
...
`tools/generate_xcodeproj.py` is what actually wrote `Yogi.xcodeproj` and can rebuild it if
it is ever mangled. Day to day, add files through Xcode.

## What is here

| Folder | Contents | Yogi screens |
|---|---|---|
...
| `Chat` | Ask Yogi (global and per item) | #57 to #59, #93 to #95 |
...
| `../YogiShare` | The "Add to Yogi" share extension | share sheet, #54 |

## Sharing into Yogi

`YogiShare` is a share extension, so "Add to Yogi" appears in every other app's share sheet
...
(`group.com.sourcedai.yogi`). Extensions get a small memory budget and no access to the
...
- A shared link becomes a real save the next time you open Yogi, not the instant you share it.
...
`Backend/YogiBackend.swift` talks to the Supabase project in `../../supabase`
...
3. Put the project URL and publishable key in
   `Yogi/Resources/Yogi.local.xcconfig` (git-ignored):
   ```
   SUPABASE_URL = https:/$()/<project-ref>.supabase.co
   SUPABASE_ANON_KEY = <publishable key>
   ```
4. `SyncEngine` starts after onboarding, pulls the library, and pushes local
   changes. Sign in with Apple, imports (`extract`) and Ask Yogi (`ask-yogi`)
...
- `SaveCover` and `CategoryIcon` use emoji on tinted grounds where Yogi shows
...
`extract` edge function, Ask Yogi goes through `ask-yogi` with chat history,
purchases go through StoreKit 2 (`com.sourcedai.yogi.pro.yearly`, with
`Resources/Yogi.storekit` attached to the run scheme for the simulator), and
