...
struct PrimaryButton: View {
    ...
    private var fill: Color {
        guard isEnabled else { return AlboColor.disabled }
        switch style {
        case .ink: return AlboColor.ink
        case .danger: return AlboColor.danger
        case .blue: return AlboColor.systemBlue
        }
    }
...
struct TextLinkButton: View {
    ...
    var body: some View {
        Button(action: onTap) {
            HStack(spacing: 4) {
                Text(prefix).foregroundStyle(AlboColor.inkSecondary)
                Text(action).fontWeight(.bold).foregroundStyle(AlboColor.ink)
            }
            .font(.alboSans(17))
        }
        .buttonStyle(.plain)
...
/// Round white floating action button with shadow (Ask Albo, dice, plus).
    ...
    var body: some View {
        Button(action: action) {
            label()
                ...
                .background(AlboColor.card, in: Circle())
