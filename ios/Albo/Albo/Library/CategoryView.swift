...
struct CategoryView: View {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    HStack(alignment: .top) {
                        VStack(alignment: .leading, spacing: 6) {
                            Text("Saved \(category.pluralTitle)").alboText(.screenTitle)
                            Text(category.librarySubtitle).font(.alboSans(16)).foregroundStyle(AlboColor.inkSecondary)
                        }
                        Spacer()
    ...
    private var topBar: some View {
        HStack {
            if selecting {
                Text("\(selected.count) selected").font(.alboSans(17, weight: .semibold)).foregroundStyle(AlboColor.ink)
                Spacer()
                Button { selecting = false; selected = [] } label: { Image(systemName: "xmark").font(.system(size: 18, weight: .semibold)).foregroundStyle(AlboColor.ink) }
            } else {
                Button { dismiss() } label: { Image(systemName: "chevron.left").font(.system(size: 22, weight: .semibold)).foregroundStyle(AlboColor.ink) }
                    .accessibilityLabel("Back")
                Spacer()
                Button { isGrid.toggle() } label: { Image(systemName: isGrid ? "list.bullet" : "square.grid.2x2").font(.system(size: 20)).foregroundStyle(AlboColor.ink) }
                    .padding(.trailing, 16)
                Button("Select") { selecting = true }.font(.alboSans(18, weight: .semibold)).foregroundStyle(AlboColor.ink)
            }
        }
        .padding(.horizontal, 20)
        .frame(height: 48)
    ...
    private func tabButton(_ t: Tab, _ title: String) -> some View {
        Button { withAnimation(.easeInOut(duration: 0.2)) { tab = t } } label: {
            VStack(spacing: 8) {
                Text(title)
                    .font(t == tab ? .alboDisplay(20) : .alboDisplayItalic(20, weight: .semibold))
                    .foregroundStyle(t == tab ? AlboColor.ink : AlboColor.muted)
                Rectangle().fill(tab == t ? AlboColor.ink : Color.clear).frame(width: 44, height: 4)
            }
        }
        .buttonStyle(.plain)
    }
        ...
        if items.isEmpty {
            if tab == .done {
                EmptyStateView(mascot: .chef, title: "You haven't completed any \(category.pluralTitle) yet",
                               message: "\(category.pluralTitle) will start to appear here as you share more things to Albo.")
                    .padding(.top, 40)
            } else {
                EmptyStateView(systemImage: "tray", title: "No \(category.pluralTitle.lowercased()) here yet",
                               message: "Share a \(category.title.lowercased()) to Albo and it will show up here.", actionTitle: "Add anything") { app.showAddSheet = true }
                    .padding(.top, 40)
            }
        } else if isGrid {
                ...
                ForEach(items) { s in
                    NavigationLink(value: LibraryRoute.save(s.id)) {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Text(s.title).font(.alboSans(16, weight: .semibold)).foregroundStyle(AlboColor.ink).lineLimit(2)
                            Text(s.recipe?.timeLabel ?? s.metaLine).font(.alboSans(13)).foregroundStyle(AlboColor.muted).lineLimit(1)
                        }
                    }
                    ...
                    if selecting {
                        Button { toggle(s.id) } label: {
                            HStack(spacing: 12) {
                                ...
                                Image(systemName: selected.contains(s.id) ? "checkmark.circle.fill" : "circle").font(.system(size: 26)).foregroundStyle(AlboColor.ink)
                            }
                        }
                        .buttonStyle(.plain)
    ...
    private var fabs: some View {
        VStack(spacing: 14) {
            FloatingButton(action: decide) { Image(systemName: "dice.fill").font(.system(size: 24)).foregroundStyle(AlboColor.ink) }
                .accessibilityLabel("Decider")
            FloatingButton(action: { app.showAddSheet = true }) { Image(systemName: "plus").font(.system(size: 26)).foregroundStyle(AlboColor.ink) }
        ...
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(save.metaLine).font(.alboSans(15)).foregroundStyle(AlboColor.inkSecondary).lineLimit(1)
                }
                if let note = save.privateNote {
                    Text(note).font(.alboSans(14)).foregroundStyle(AlboColor.ink)
                        .padding(.horizontal, 12).frame(height: 32)
                        .background(AlboColor.card, in: RoundedRectangle(cornerRadius: 10, style: .continuous))
            ...
            if let onMenu {
                Button(action: onMenu) {
                    Image(systemName: "ellipsis").font(.system(size: 20, weight: .bold)).foregroundStyle(AlboColor.ink).frame(width: 36, height: 36)
                }
                .buttonStyle(.plain)
...
struct DeciderResult: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("The Decider says").font(.alboSans(15)).foregroundStyle(AlboColor.muted)
            Text(save.title).alboText(.sheetTitle).multilineTextAlignment(.center)
            Text(save.metaLine).font(.alboSans(16)).foregroundStyle(AlboColor.inkSecondary)
            Spacer()
