...
struct SaveCollection: Identifiable, Hashable, Codable {
    ...
    var saveIDs: [UUID] = []
    var inviteURL: URL? = URL(string: "https://join.albo.inc/invite")
}
...
enum OnboardingGoal: String, CaseIterable, Identifiable, Codable, Hashable {
    ...
    var reinforcement: String {
        switch self {
        case .doThings: return "Albo turns every save into something you can actually tick off."
        case .onePlace: return "Albo pulls saves from TikTok, Instagram, Safari and screenshots into one library."
        case .map: return "Every place you save gets pinned on a map, ready for your next trip."
        case .recipes: return "Albo pulls ingredients and step-by-step instructions straight out of your TikTok recipes."
        case .stopLosing: return "Share once to Albo and it's found, filed and searchable forever."
...
enum ContentInterest: String, CaseIterable, Identifiable, Codable, Hashable {
    ...
    /// "Setting up your Albo for" checklist line (Albo #37).
    var planLine: String {
        switch self {
