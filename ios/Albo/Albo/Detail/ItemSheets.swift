...
struct ItemActionsSheet: View {
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text(save.title).font(.alboSans(18, weight: .bold)).foregroundStyle(AlboColor.ink).lineLimit(1)
                        Text(save.metaLine).font(.alboSans(14)).foregroundStyle(AlboColor.inkSecondary).lineLimit(1)
                    }
                    Spacer()
                ...
                HStack(spacing: 14) {
                    ShareLink(item: save.sourceURL ?? URL(string: "https://albo.inc")!) { bigSquare("square.and.arrow.up", "Share") }
                ...
                Button { confirmRemove = true } label: {
                    HStack(spacing: 14) {
                        ...
                        Text("Remove from All My Saves").font(.alboSans(17, weight: .medium))
                        Spacer()
                    }
                    .foregroundStyle(AlboColor.danger).padding(.horizontal, 18).frame(height: 58)
                    .background(AlboColor.surface, in: RoundedRectangle(cornerRadius: 18, style: .continuous))
                }
                .buttonStyle(PressableButtonStyle())
                Spacer(minLength: 0)
    ...
    private func bigSquare(_ symbol: String, _ title: String) -> some View {
        VStack(spacing: 10) {
            ...
            Text(title).font(.alboSans(16, weight: .medium))
        }
    ...
    private func row(_ symbol: String, _ title: String, _ action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 14) {
                Image(systemName: symbol).font(.system(size: 20)).foregroundStyle(AlboColor.ink).frame(width: 28)
                Text(title).font(.alboSans(17, weight: .medium)).foregroundStyle(AlboColor.ink)
                Spacer()
            }
...
struct ReminderSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            Capsule().fill(AlboColor.hairline).frame(width: 40, height: 5).frame(maxWidth: .infinity).padding(.top, 8)
            Text("Set a Reminder").font(.alboSans(24, weight: .bold)).foregroundStyle(AlboColor.ink)
            HStack(spacing: 12) {
                Text("Remind me in the").font(.alboSans(17)).foregroundStyle(AlboColor.inkSecondary)
                Menu {
                    ForEach(ReminderSlot.allCases) { s in Button(s.menuLabel) { slot = s } }
                } label: {
                    HStack(spacing: 8) {
                        Image(systemName: "sunrise").font(.system(size: 16))
                        Text(slot.menuLabel).font(.alboSans(16, weight: .semibold))
                        Image(systemName: "chevron.down").font(.system(size: 12, weight: .semibold))
                    }
                    .foregroundStyle(AlboColor.ink).padding(.horizontal, 14).frame(height: 40).background(AlboColor.optionFill, in: Capsule())
                }
            }
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ForEach(groups, id: \.0) { name, days in
                        VStack(alignment: .leading, spacing: 10) {
                            Text(name).font(.alboSans(15, weight: .semibold)).foregroundStyle(AlboColor.inkSecondary)
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 10) {
                                    ForEach(days, id: \.self) { d in
                                        Button { set(d) } label: {
                                            VStack(spacing: 4) {
                                                Text(d, format: .dateTime.weekday(.abbreviated)).font(.alboSans(14)).foregroundStyle(AlboColor.muted)
                                                Text(d, format: .dateTime.day()).font(.alboSans(20, weight: .bold)).foregroundStyle(AlboColor.ink)
                                                Text(d, format: .dateTime.month(.abbreviated)).font(.alboSans(13)).foregroundStyle(AlboColor.muted)
                                            }
                                            .frame(width: 72, height: 84)
                                            .background(AlboColor.card, in: RoundedRectangle(cornerRadius: 14, style: .continuous))
                                            .overlay(RoundedRectangle(cornerRadius: 14, style: .continuous).stroke(AlboColor.hairline, lineWidth: 1))
                                        }
                                        .buttonStyle(PressableButtonStyle())
                                    }
                                }
            ...
            Button { showCustom = true } label: {
                HStack(spacing: 10) { Image(systemName: "calendar"); Text("Custom date & time") }
                    .font(.alboSans(17, weight: .semibold)).foregroundStyle(AlboColor.ink)
...
struct AddToCollectionSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            Capsule().fill(AlboColor.hairline).frame(width: 40, height: 5).frame(maxWidth: .infinity).padding(.top, 8)
            HStack(spacing: 14) {
                Image(systemName: "bookmark.fill").font(.system(size: 22)).foregroundStyle(AlboColor.ink)
                Text("All My Saves").font(.alboSans(18, weight: .bold)).foregroundStyle(AlboColor.ink)
                Spacer()
                Image(systemName: "checkmark.circle.fill").font(.system(size: 26)).foregroundStyle(AlboColor.ink)
            }
            .padding(.horizontal, 4)
            Divider()
            Text("Add to a collection").font(.alboSans(15, weight: .semibold)).foregroundStyle(AlboColor.inkSecondary)
            Button { showNew = true } label: {
                HStack(spacing: 14) {
                    Image(systemName: "plus").font(.system(size: 20, weight: .medium)).frame(width: 52, height: 52).background(AlboColor.optionFill, in: RoundedRectangle(cornerRadius: 12))
                    Text("New collection").font(.alboSans(18, weight: .semibold))
                    Spacer()
                }
                .foregroundStyle(AlboColor.ink)
            }
                    ...
                    ForEach(app.collections) { c in
                        ...
                        Button {
                            ...
                            HStack(spacing: 14) {
                                ...
                                Text(c.name).font(.alboSans(18, weight: .semibold)).foregroundStyle(AlboColor.ink)
                                Spacer()
                                Image(systemName: inAll ? "checkmark.circle.fill" : "plus.circle").font(.system(size: 26)).foregroundStyle(AlboColor.ink)
                            }
                        }
                        .buttonStyle(PressableButtonStyle())
                    }
        ...
        VStack(spacing: 0) {
            HStack {
                Button { if step == 1 { step = 0 } else { dismiss() } } label: {
                    Image(systemName: step == 1 ? "arrow.left" : "xmark").font(.system(size: 22, weight: .medium)).foregroundStyle(AlboColor.ink).frame(width: 40, height: 40)
                }
                .buttonStyle(.plain)
                Spacer()
                Text("New Collection").font(.alboSans(20, weight: .bold)).foregroundStyle(AlboColor.ink)
                Spacer()
                switch step {
                case 0:
                    Button("Next") { step = 1 }.font(.alboSans(18, weight: .semibold)).foregroundStyle(name.isEmpty ? AlboColor.muted : AlboColor.ink).disabled(name.isEmpty).frame(width: 60)
                case 1:
                    Button { create() } label: {
                        Text("Done").font(.alboSans(17, weight: .semibold)).foregroundStyle(.white).padding(.horizontal, 18).frame(height: 44)
                    ...
                    .buttonStyle(PressableButtonStyle())
                default:
                    Button("Close") { dismiss() }.font(.alboSans(18, weight: .semibold)).foregroundStyle(AlboColor.ink).frame(width: 60)
                }
            }
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                case 1:
                    Menu {
                        ...
                        HStack {
                            Text(isPublic ? "Public - Anyone can see" : "Private - Only you").font(.alboSans(18)).foregroundStyle(AlboColor.ink)
                            Spacer()
                ...
                default:
                    Text("Invite Friends").font(.alboSans(30, weight: .bold)).foregroundStyle(AlboColor.ink)
                    if let c = created, let url = c.inviteURL {
                        ShareLink(item: url) {
                            ZStack {
                                Capsule().fill(Color.black).offset(y: 5)
                                Capsule().fill(AlboColor.ink)
                                Text("Create & Share Invite").font(.alboSans(18, weight: .semibold)).foregroundStyle(.white)
                            }
...
struct TagFriendsSheet: View {
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            Text("Tag people you did this with").font(.alboSans(20, weight: .bold)).foregroundStyle(AlboColor.ink)
                            ...
                            Button {
                                ...
                                HStack(spacing: 14) {
                                    ...
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text(u.name).font(.alboSans(17, weight: .bold)).foregroundStyle(AlboColor.ink)
                                        Text(u.handle).font(.alboSans(15)).foregroundStyle(AlboColor.muted)
                                    }
                                    Spacer()
                                    Image(systemName: on ? "checkmark.circle.fill" : "circle").font(.system(size: 26)).foregroundStyle(AlboColor.ink)
                                }
                            }
                            .buttonStyle(.plain)
                        }
