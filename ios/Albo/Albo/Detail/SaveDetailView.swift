...
struct SaveDetailView: View {
    ...
    private func content(_ save: Save) -> some View {
        ZStack(alignment: .bottomTrailing) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    if save.coverEmoji != nil && save.category != .note {
                        ...
                        ZStack(alignment: .topLeading) {
                            ...
                            Text(save.title).alboText(.itemTitle).padding(.horizontal, 20).padding(.top, 12)
                        }
                    }
            ...
            AskAlboFAB { showAsk = true }.padding(.trailing, 20).padding(.bottom, 20)
        }
        ...
        .sheet(isPresented: $showAsk) { AskAlboSheet(scope: save) }
    ...
    private func topBar(_ save: Save) -> some View {
        HStack(spacing: 12) {
            Button { dismiss() } label: { Image(systemName: "chevron.left").font(.system(size: 22, weight: .semibold)).foregroundStyle(AlboColor.ink).frame(width: 36, height: 36) }
                .buttonStyle(.plain).accessibilityLabel("Back")
            Text(save.title).font(.alboSans(20, weight: .bold)).foregroundStyle(AlboColor.ink).lineLimit(1)
                .opacity(save.coverEmoji == nil || save.category == .note ? 0 : 1)
            Spacer()
            ShareLink(item: save.sourceURL ?? URL(string: "https://albo.inc")!) {
                Image(systemName: "square.and.arrow.up").font(.system(size: 22)).foregroundStyle(AlboColor.ink).frame(width: 36, height: 36)
            }
            Button { showActions = true } label: { Image(systemName: "ellipsis").font(.system(size: 22, weight: .bold)).foregroundStyle(AlboColor.ink).rotationEffect(.degrees(90)).frame(width: 36, height: 36) }
                .buttonStyle(.plain).accessibilityLabel("More")
        }
        .padding(.horizontal, 12)
        .frame(height: 48)
        .background(save.coverEmoji == nil || save.category == .note ? AlboColor.ground : Color.clear)
    }
    ...
    private func metaBlock(_ save: Save) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 10) {
                ...
                Text(save.metaLine).font(.alboSans(18)).foregroundStyle(AlboColor.inkSecondary).lineLimit(2)
            }
            HStack(spacing: 10) {
                if !save.savedBy.isEmpty { AvatarStack(users: save.savedBy) }
                Text("\(save.saveCount) save\(save.saveCount == 1 ? "" : "s")").font(.alboSans(16)).foregroundStyle(AlboColor.inkSecondary)
                ForEach(save.reactions) { r in
                    HStack(spacing: 4) { Text(r.emoji); Text("\(r.count)") }.font(.alboSans(14)).foregroundStyle(AlboColor.ink)
                        .padding(.horizontal, 10).frame(height: 32).background(AlboColor.optionFill, in: Capsule())
                }
                Spacer(minLength: 4)
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                HStack(spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(review.sentiment.badgeTitle).font(.alboSans(17, weight: .bold)).foregroundStyle(AlboColor.ink)
                        Text(review.createdAt, style: .relative).font(.alboSans(13)).foregroundStyle(AlboColor.muted)
                    }
                    Button { showReview = true } label: { Image(systemName: "arrow.clockwise").font(.system(size: 14, weight: .semibold)).foregroundStyle(AlboColor.inkSecondary) }
                        .buttonStyle(.plain)
                }
            ...
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    if let t = review.title { Text(t).font(.alboSans(15)).foregroundStyle(AlboColor.inkSecondary) }
                }
                Spacer()
    ...
    private func recipeBody(_ save: Save) -> some View {
        if let r = save.recipe, let binding = app.binding(for: save.id) {
            if let yield = r.yieldLabel {
                HStack(spacing: 12) {
                    Image(systemName: "fork.knife").font(.system(size: 20)).foregroundStyle(AlboColor.inkSecondary)
                    Text(yield).font(.alboSans(18)).foregroundStyle(AlboColor.inkSecondary)
                }
            }
            VStack(alignment: .leading, spacing: 16) {
                ...
                if r.ingredients.isEmpty {
                    Text("No ingredients were extracted. Use Reanalyze on the source note or report a bad extract.")
                        .font(.alboSans(15)).foregroundStyle(AlboColor.muted)
                } else {
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    ForEach(r.steps.indices, id: \.self) { i in
                        HStack(alignment: .top, spacing: 16) {
                            ...
                            Text(r.steps[i]).font(.alboSans(18)).foregroundStyle(AlboColor.ink).fixedSize(horizontal: false, vertical: true)
                        }
                    }
                }
            }
    ...
    private func placeBody(_ save: Save) -> some View {
        if let p = save.place {
            ...
            if let hours = p.hoursLabel {
                HStack { Text(hours).font(.alboSans(18)).foregroundStyle(AlboColor.inkSecondary); Image(systemName: "chevron.right").font(.system(size: 14, weight: .semibold)).foregroundStyle(AlboColor.inkSecondary) }
            } else {
                Text("No opening times found").font(.alboSans(18)).foregroundStyle(AlboColor.muted)
            }
    ...
    private func chipLabel(_ symbol: String, _ title: String) -> some View {
        HStack(spacing: 8) { Image(systemName: symbol).font(.system(size: 16)); Text(title).font(.alboSans(17, weight: .medium)) }
            .foregroundStyle(AlboColor.ink).padding(.horizontal, 18).frame(height: 52)
    ...
    private func eventBody(_ save: Save) -> some View {
        if let e = save.event {
            if let about = e.about { Text(about).font(.alboSans(17)).foregroundStyle(AlboColor.inkSecondary) }
            HStack(spacing: 12) {
                Image(systemName: "calendar").font(.system(size: 20)).foregroundStyle(AlboColor.inkSecondary)
                Text("Starts on \(e.start.formatted(.dateTime.month(.abbreviated).day().year())) at \(e.start.formatted(.dateTime.hour().minute()))").font(.alboSans(17)).foregroundStyle(AlboColor.ink)
            }
            if let address = e.address {
                HStack(alignment: .top, spacing: 12) {
                    Image(systemName: "mappin.and.ellipse").font(.system(size: 20)).foregroundStyle(AlboColor.inkSecondary)
                    Text(address).font(.alboSans(17)).foregroundStyle(AlboColor.ink)
                }
            }
    ...
    private func noteBody(_ save: Save) -> some View {
        if let body = save.noteBody {
            if let attributed = try? AttributedString(markdown: body, options: .init(interpretedSyntax: .inlineOnlyPreservingWhitespace)) {
                Text(attributed).font(.alboSans(18)).foregroundStyle(AlboColor.ink)
            } else {
                Text(body).font(.alboSans(18)).foregroundStyle(AlboColor.ink)
            }
    ...
    private func genericBody(_ save: Save) -> some View {
        ...
        if let url = save.sourceURL {
            Link(destination: url) {
                HStack(spacing: 10) {
                    ...
                    Text(url.host ?? "Open link").font(.alboSans(17, weight: .medium))
                    Spacer()
        ...
        if let subtitle = save.subtitle, save.media?.author == nil {
            Text(subtitle).font(.alboSans(17)).foregroundStyle(AlboColor.inkSecondary)
        }
    }
    ...
    private func mentionedIn(_ save: Save) -> some View {
        if let noteID = save.mentionedInNoteID, let note = app.save(noteID) {
            VStack(alignment: .leading, spacing: 12) {
                Text("Mentioned in").font(.alboSans(20, weight: .bold)).foregroundStyle(AlboColor.ink)
                NavigationLink(value: LibraryRoute.save(note.id)) {
                    VStack(alignment: .leading, spacing: 8) {
                        HStack { Spacer(); Image(systemName: "bookmark.fill").foregroundStyle(AlboColor.hairline) }
                        Text(note.title).font(.alboSans(16, weight: .bold)).foregroundStyle(AlboColor.ink).multilineTextAlignment(.leading)
                        Text(note.noteBody ?? "").font(.alboSans(14)).foregroundStyle(AlboColor.inkSecondary).lineLimit(3).multilineTextAlignment(.leading)
                        Spacer(minLength: 0)
                    }
                    .padding(14)
                    .frame(width: 130, height: 170, alignment: .topLeading)
                    .background(AlboColor.surface, in: RoundedRectangle(cornerRadius: 16, style: .continuous))
                    .shadow(color: .black.opacity(0.06), radius: 10, y: 4)
                }
    ...
    private func notesBlock(_ save: Save) -> some View {
        if let note = save.privateNote, !editingNote {
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 6) { Image(systemName: "lock").font(.system(size: 13)); Text("Private note").font(.alboSans(15)) }.foregroundStyle(AlboColor.muted)
                Text(note).font(.alboSans(18)).foregroundStyle(AlboColor.ink)
                Text("a moment ago").font(.alboSans(14)).foregroundStyle(AlboColor.muted)
            }
            .padding(18)
            ...
            HStack {
                TextField("Add a note...", text: $noteDraft).font(.alboSans(18))
                Button {
                    app.setPrivateNote(noteDraft, for: save.id)
                    editingNote = false
                } label: { Image(systemName: "plus").font(.system(size: 22)).foregroundStyle(AlboColor.ink) }
                .buttonStyle(.plain)
    ...
    private func commentsBlock(_ save: Save) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 10) {
                Text("Comments & Reviews").font(.alboSans(22, weight: .bold)).foregroundStyle(AlboColor.ink)
                if !save.comments.isEmpty { Text("\(save.comments.count)").font(.alboSans(17)).foregroundStyle(AlboColor.inkSecondary) }
            }
            HStack(spacing: 12) {
                AvatarView(user: app.me, size: 44)
                HStack {
                    TextField("Add a comment...", text: $commentDraft).font(.alboSans(17))
                    Button {
                        app.addComment(commentDraft, to: save.id)
                        commentDraft = ""
                    } label: {
                        Image(systemName: "arrow.up").font(.system(size: 18, weight: .bold)).foregroundStyle(.white).frame(width: 44, height: 44).background(AlboColor.inkSecondary, in: Circle())
                    }
                    .buttonStyle(.plain)
            ...
            if save.comments.isEmpty {
                Text("Be the first to comment!").font(.alboSans(17)).foregroundStyle(AlboColor.muted)
            } else {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        HStack(alignment: .top, spacing: 12) {
                            ...
                            VStack(alignment: .leading, spacing: 3) {
                                HStack(spacing: 6) {
                                    Text(c.author.handle).font(.alboSans(17, weight: .bold)).foregroundStyle(AlboColor.ink)
                                    Text(c.createdAt, style: .relative).font(.alboSans(14)).foregroundStyle(AlboColor.muted) + Text(" ago").font(.alboSans(14)).foregroundStyle(AlboColor.muted)
                                }
                                Text(save.title).font(.alboSans(15)).foregroundStyle(AlboColor.muted)
                        ...
                        .font(.system(size: 20)).foregroundStyle(AlboColor.ink).padding(.leading, 56)
                        Divider()
                    }
                }
            }
...
struct HeroHeader: View {
    ...
    var body: some View {
        ZStack(alignment: .bottomLeading) {
            ...
            LinearGradient(colors: [.clear, AlboColor.ground.opacity(0.6), AlboColor.ground], startPoint: .center, endPoint: .bottom).frame(height: 320)
            ...
            Text(save.title).alboText(.itemTitle).padding(.horizontal, 20).padding(.bottom, 4)
        }
        .frame(height: 320)
        .clipped()
    }
                    ...
                    HStack(spacing: 22) {
                        ForEach(0..<cols, id: \.self) { col in
                            Image(systemName: (row + col) % 2 == 0 ? "fork.knife" : "cup.and.saucer")
                                .font(.system(size: 16))
                                .foregroundStyle(AlboColor.hairline)
                        }
                    }
...
struct ServingMultiplier: View {
    ...
    var body: some View {
        HStack {
            HStack(spacing: 14) {
                ...
                Text("\(multiplier)×").font(.alboSans(20, weight: .bold)).foregroundStyle(AlboColor.ink).monospacedDigit()
            ...
            if showsConvert, let converted {
                Button { converted.wrappedValue.toggle() } label: {
                    HStack(spacing: 6) {
                        ...
                        Text(converted.wrappedValue ? "Original" : "Convert").font(.alboSans(17, weight: .medium))
                    }
                    .foregroundStyle(AlboColor.ink).padding(.horizontal, 18).frame(height: 48)
                    .background(Capsule().stroke(AlboColor.hairline, lineWidth: 1.5))
                }
                .buttonStyle(PressableButtonStyle())
            }
    ...
    private func stepper(_ symbol: String, _ action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Image(systemName: symbol).font(.system(size: 18, weight: .medium)).foregroundStyle(AlboColor.ink)
                .frame(width: 48, height: 48).background(AlboColor.optionFill, in: Circle())
        }
        .buttonStyle(PressableButtonStyle())
    }
}
        ...
        Button(action: onToggle) {
            HStack(spacing: 14) {
                Image(systemName: ingredient.checked ? "checkmark.square.fill" : "square")
                    ...
                    .foregroundStyle(ingredient.checked ? AlboColor.ink : AlboColor.inkSecondary)
                ...
                (Text(ingredient.name).fontWeight(.bold) + Text(quantity.map { " - \($0)" } ?? ""))
                    .font(.alboSans(18))
                    .foregroundStyle(ingredient.checked ? AlboColor.muted : AlboColor.ink)
                    .strikethrough(ingredient.checked, color: AlboColor.muted)
        ...
        Button(action: onTap) {
            Map(initialPosition: .region(MKCoordinateRegion(center: coord, span: MKCoordinateSpan(latitudeDelta: 0.02, longitudeDelta: 0.02)))) {
                Annotation("", coordinate: coord) {
                    Text(emoji).font(.system(size: 22)).padding(6).background(AlboColor.card, in: Circle()).shadow(color: .black.opacity(0.2), radius: 4, y: 2)
                }
            }
            ...
            .overlay(alignment: .topTrailing) {
                if let label { Text(label).font(.alboSans(12, weight: .semibold)).tracking(1).foregroundStyle(AlboColor.inkSecondary).padding(10) }
            }
            .overlay(alignment: .topLeading) {
                Image(systemName: "globe").font(.system(size: 18)).foregroundStyle(AlboColor.ink).padding(8).background(AlboColor.card, in: Circle()).padding(10)
            }
        }
        .buttonStyle(.plain)
    }
        ...
        ZStack(alignment: .topTrailing) {
            Map(initialPosition: .region(MKCoordinateRegion(center: coord, span: MKCoordinateSpan(latitudeDelta: 0.03, longitudeDelta: 0.03)))) {
                Annotation(save.title, coordinate: coord) {
                    Text(save.place?.emoji ?? "📍").font(.system(size: 24)).padding(8).background(AlboColor.card, in: Circle()).shadow(color: .black.opacity(0.2), radius: 5, y: 2)
                }
            }
            .ignoresSafeArea()
            Button { dismiss() } label: { Image(systemName: "xmark").font(.system(size: 18, weight: .semibold)).foregroundStyle(AlboColor.ink).frame(width: 44, height: 44).background(AlboColor.card, in: Circle()) }
                .padding(20)
        }
    }
}
