...
// Config: SUPABASE_URL and SUPABASE_ANON_KEY in Resources/Albo.xcconfig, surfaced through Info.plist.
...
enum BackendError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notConfigured: return "Backend is not configured. Add SUPABASE_URL and SUPABASE_ANON_KEY to Albo.xcconfig."
...
@MainActor
final class AlboBackend {
    static let shared = AlboBackend()
    ...
    /// Ask Albo (Albo #57, #93). Throws `.proRequired` on 402.
    func ask(_ question: String, saveID: UUID?, history: [ChatMessage]) async throws -> String {
        ...
        do {
            let response: AskResponse = try await db().functions.invoke("ask-albo", options: FunctionInvokeOptions(body: request), decoder: decoder)
...
/// calling `SyncEngine.shared.start(app:)` from `AlboApp` once the package is added.
@MainActor
final class SyncEngine {
    ...
    func start(app: AppState) {
        guard AlboBackend.shared.isConfigured else { return }
        task?.cancel()
        task = Task {
            await pull(into: app)
        }
    ...
    func pull(into app: AppState) async {
        do {
            let backend = AlboBackend.shared
            ...
            if !collections.isEmpty {
                app.collections = collections.map { c in
                    SaveCollection(id: c.id, name: c.name, details: c.details, coverEmoji: c.coverEmoji, coverTint: UInt32(max(0, c.coverTint)), isPublic: c.isPublic,
                                   owner: app.me, saveIDs: membership.filter { $0.0 == c.id }.map(\.1), inviteURL: c.inviteToken.flatMap { URL(string: "https://join.albo.inc/\($0)") })
                }
            }
    ...
    func push(_ save: Save) {
        guard AlboBackend.shared.isConfigured else { return }
        Task { try? await AlboBackend.shared.upsert(save) }
    }
}
#endif
