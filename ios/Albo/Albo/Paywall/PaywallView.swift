        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Button(action: onDismiss) {
                    Image(systemName: "arrow.left").font(.system(size: 22, weight: .medium)).foregroundStyle(AlboColor.ink).frame(width: 36, height: 36)
                }
                .buttonStyle(.plain)
                .accessibilityLabel("Back")
                Spacer()
                Button(action: onDismiss) {
                    Image(systemName: "xmark").font(.system(size: 20, weight: .medium)).foregroundStyle(AlboColor.ink).frame(width: 36, height: 36)
                }
                .buttonStyle(.plain)
            ...
            Text("How the trial works").alboText(.onboardingHeadline).padding(.horizontal, 24).padding(.top, 12)

            VStack(alignment: .leading, spacing: 0) {
                TimelineRow(color: AlboColor.rewardGreen, symbol: "lock.open.fill", title: "Today",
                            text: "Unlock access to all the app's features like unlimited saves & bulk importing saves from other apps.", isLast: false)
                TimelineRow(color: AlboColor.danger, symbol: "bell.fill", title: "In 2 Days - Reminder",
                            text: "We'll send you a reminder that your trial is ending soon.", isLast: false)
                TimelineRow(color: AlboColor.optionFill, symbol: "crown.fill", symbolTint: AlboColor.star, title: "In 3 Days - Billing Starts",
            ...
            VStack(spacing: 14) {
                ...
                Text("\(PurchaseService.trialDays) days free, then \(PurchaseService.yearlyPrice) per year")
                    .font(.alboSans(17)).foregroundStyle(AlboColor.inkSecondary)
            }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 18) {
            VStack(spacing: 0) {
                ...
                if !isLast {
                    Rectangle().fill(AlboColor.hairline).frame(width: 2).frame(maxHeight: .infinity)
                }
            }
            VStack(alignment: .leading, spacing: 6) {
                Text(title).font(.alboSans(20, weight: .bold)).foregroundStyle(AlboColor.ink)
                Text(text).font(.alboSans(17)).foregroundStyle(AlboColor.inkSecondary).fixedSize(horizontal: false, vertical: true)
            }
...
struct ManageSubscriptionSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Text("Manage Subscription").alboText(.onboardingHeadline)
            Text("Choose your plan").font(.alboSans(17)).foregroundStyle(AlboColor.inkSecondary)
        ...
        Button(action: onTap) {
            HStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(subtitle).font(.alboSans(15)).foregroundStyle(AlboColor.inkSecondary).multilineTextAlignment(.leading)
                }
                Spacer()
                Image(systemName: isSelected ? "checkmark.circle.fill" : "chevron.right")
                    .font(.system(size: isSelected ? 26 : 16, weight: .semibold)).foregroundStyle(AlboColor.ink)
            }
            .padding(18)
            .background(AlboColor.surface, in: RoundedRectangle(cornerRadius: 22, style: .continuous))
            .overlay(RoundedRectangle(cornerRadius: 22, style: .continuous).stroke(AlboColor.ink, lineWidth: isSelected ? 0 : 1.5))
        }
